...
{
	FMC_Bank5_6_R->SDCMR =	((mode_register << 9) & FMC_SDCMR_MRD) |
							(((auto_refresh - 1) << 5) & FMC_SDCMR_NRFS) |
							FMC_SDCMR_CTB1 | (mode & FMC_SDCMR_MODE);

	int t = 1024;
	for (; (FMC_Bank5_6_R->SDSR & FMC_SDSR_BUSY) and t; t--) {
		modm::delay_ms(1);
	}
...
board_initialize_sdram()
{
	 ...
	 * - NC: 0b00 (8 bit)
	 */
	FMC_Bank5_6_R->SDCR[0] =  FMC_SDCRx_RBURST | FMC_SDCRx_SDCLK_1 | FMC_SDCRx_NB | FMC_SDCRx_NR_0 |
							FMC_SDCRx_MWID_1 | FMC_SDCRx_CAS_1 | FMC_SDCRx_CAS_0;
	 ...
	 */
	FMC_Bank5_6_R->SDTR[0] = 0x01116361;
